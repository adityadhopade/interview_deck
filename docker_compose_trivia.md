...
- But as ***Concept of Service is not inherent to Docker so we need to explicitly need to create the network; then Docker realises and allows us the DNS mapping (Name to IP Mapping)*** 
...
- It will not run as the PORTS will get occupied by the first instance of base; when scaled it will also try to run on the sme PORT but as we know the port once allocated cannot be ocupied by other scaled instances. 
```
# Stop docker compose
